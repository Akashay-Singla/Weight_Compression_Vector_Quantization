...
{
int d[4];//distortion
int mean_sqr_err[8]; //mean square error
int i=0, num=0;
...
for(int8_t row=0;row<8;row=row+2)
{
	    for(int8_t col=0;col<8;col=col+2)
	{
          ...
          mean_sqr_err[cd_row] = (int)((d[0]+d[1]+d[2]+d[3])/4);
...
for(int cod_row=0;cod_row<8;cod_row++)
{
	sum[0]=0;sum[1]=0;sum[2]=0; sum[3]=0;
	count=0;
for(int addr_row =0;addr_row<4;addr_row++)
{
	for(int addr_col=0;addr_col<4;addr_col++)
	{       //It matches the address book value with row of the codebook
            if(addr_book[addr_row][addr_col] == cod_row)
            {
            	...
            	sum[0] += input_vec[addr_row *2][addr_col *2];
            	sum[1] += input_vec[(addr_row*2)][(addr_col*2)+1];
            	sum[2] += input_vec[(addr_row*2)+1][(addr_col*2)];
            	sum[3] += input_vec[(addr_row*2)+1][(addr_col*2)+1];
            	count++;
            	flag++;
            }
	}
            ...
            if(flag){
            	...
            	sum_temp[2]= ceil((double)sum[2]/(double)count);
            	sum_temp[3]= ceil((double)sum[3]/(double)count);
            	...
            	diff[2]= cdbook[cod_row][2] - sum_temp[2];
            	diff[3]= cdbook[cod_row][3] - sum_temp[3];
            	if(diff[0]>max) max=diff[0];
            	else if(diff[1]>max) max=diff[1];
            	else if(diff[2]>max) max=diff[2];
            	else if(diff[3]>max) max=diff[3];

            	//updating the codebook values
            	cdbook[cod_row][0]= sum_temp[0];
            	cdbook[cod_row][1]= sum_temp[1] ;
            	cdbook[cod_row][2]= sum_temp[2];
            	cdbook[cod_row][3]= sum_temp[3];
            	flag=0;
            }
}
//Akashay Singla
